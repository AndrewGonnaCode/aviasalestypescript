...
const renderPrice = (price:number | string[], value:number) => {
    let arr:any[] = [];
...
const mapMinutesToHours = (minutes:number) => {
    return `${Math.floor(minutes / 60)}ч ${minutes % 60}м`;
};

const formateStops = (value:number) => {
...
const formateRouteTime = (date:string, duration:number) => {
        ...
        <div className="tickets__ticket ticket">
            ...
            <div className="ticket__header">
                <div className="ticket__price">
                    {
                        renderPrice(ticket.price, 3)
                    } Р
                </div>
                <div className="ticket__carrier">
                    <img src={`http://pics.avs.io/99/36/${ticket.carrier}.png`} alt="S7" />
                </div>
            </div>
